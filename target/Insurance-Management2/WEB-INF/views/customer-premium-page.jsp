<html>
  ...
  <body>
    ...
    <div class="container p-5 bg-light">
      <h2 class="text-center mt-3 mb-5">PREMIUM PAID STATEMENT</h2>
            ...
            <tbody>
              <c:forEach var="c" items="${custpolicyobject}" varStatus="status">
                <tr>
                  ...
                  <td>${c.getCust_id()}</td>
                  <td>${c.getPolicy_number()}</td>
                  <td>${c.getPolicy_name()}</td>
                  <td>${c.getSum_assured_needed()}</td>
                  <td>${c.getNo_of_years()}</td>
                  <td>${c.getPremium_payment_cycle()}</td>
                  <td>${c.getCalculated_premium()}</td>
                </tr>
              </c:forEach>
            </tbody>
          </table>
